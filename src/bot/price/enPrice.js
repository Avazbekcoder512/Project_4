...
async function sendEnPrice(ctx, page = 1, editMessage = false) {
    try {
        ...
        if (result.hasPrevPage) keyboard.text('⬅️ Previous page', `sheet_${result.prevPage}`);
        if (result.hasNextPage) keyboard.text('Next page ➡️', `sheet_${result.nextPage}`);
    ...
    try {
        ...
        if (data.startsWith('sheet_')) {
            ...
            if (section) {
                ...
                const keyboard = new InlineKeyboard().text('⬅️ Back', `back_to_sections_${page}`);
